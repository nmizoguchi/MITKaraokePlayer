...
public class ChordTest {
	...
	public void testGetMethods() {
		...
		// check to see if the durationMultiplier of the second Note in the Chord was changed properly
	...
	public void testMultiplyDurationMultiplier() {
		...
		assertEquals(2.0/3.0*2, chord.getListOfNotesInChord().get(0).getDurationMultiplier(), 0.00001);
		// Check that the 2nd Note in the Chord  has a durationMultiplier equal to 2.0/3.0*2 
		assertEquals(chord.getDurationMultiplier(), chord.getListOfNotesInChord().get(1).getDurationMultiplier(), 0.00001);
	}	
}